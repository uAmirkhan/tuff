...
const vidno = (sel) => page.evaluate((s) => { const e = document.querySelector(s); return !!e && getComputedStyle(e).display !== 'none' && e.classList.contains('pokazan'); }, sel);
...
const stroki = await page.$$eval('#menyu .uroven', (els) => els.map((e) => `${e.textContent?.trim().slice(0, 30)}${e.classList.contains('zakryt') ? ' [закрыт]' : ''}`));
...
await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: [{ x: vp.width * 0.25, y: vp.height * 0.6, id: 0 }] });
await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [{ x: vp.width * 0.25 + 60, y: vp.height * 0.6, id: 0 }] });
