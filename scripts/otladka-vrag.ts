...
const telo = new Telo(mir, 2, 0.6);
const igra = new Igra(mir, telo, ur);
const v = igra.vragi[0]!;
for (let t = 0; t < 480; t++) {
  igra.doShaga();
  telo.primenit(PUSTOE, false);
  mir.shag();
  telo.posle(PUSTOE);
  igra.takt(PUSTOE);
  if (t % 60 === 59) {
    ...
    let kt = 0,
      kv = 0;
    for (let i = telo.ot; i < telo.ot + telo.n; i++) if (mir.kontaktTel[i]) kt++;
    for (let i = v.ot; i < v.ot + v.n; i++) if (mir.kontaktTel[i]) kv++;
    const g = v.gabarity();
