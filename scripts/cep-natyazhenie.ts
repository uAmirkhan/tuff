...
for (const zapas of [1.03, 1.06, 1.1])
  for (const korka of [false, true]) {
    ...
    for (let t = 0; t < 600; t++) {
      ...
      igra.takt(nam);
      if (naMostu)
        for (const sv of m.svyazi) {
          if (!mir.sZhiva[sv]) continue;
          const a = mir.sA[sv]!,
            b = mir.sB[sv]!;
          const d = Math.hypot(mir.x[b]! - mir.x[a]!, mir.y[b]! - mir.y[a]!);
          maxStretch = Math.max(maxStretch, d / mir.sDlina[sv]! - 1);
        }
    }
    telo.schitatCentr();
    console.log(
