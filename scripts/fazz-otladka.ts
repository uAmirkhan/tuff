...
for (const id of ['1-4', '1-5']) {
  ...
  for (let t = 0; t < 4000; t++) {
    ...
    for (const s of igra.sobytiya)
      if (s.tip === 'storozh') {
        prichiny.set(s.prichina, (prichiny.get(s.prichina) ?? 0) + 1);
        if (gde.length < 6) {
          telo.schitatCentr();
          const vr = igra.vragi
            .filter((v) => v.zhiv)
          ...
          gde.push(
            `t${t} x${telo.cx.toFixed(1)} y${telo.cy.toFixed(1)} ${JSON.stringify(nam)} враги ${vr.join(' ')}`,
          );
        }
      }
  }
  console.log(id, Object.fromEntries(prichiny));
  for (const g of gde) console.log('  ', g);
}
