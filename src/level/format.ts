...
export type TipObekta =
  ...
  | 'voda' // зона: урон и принудительная Корка; с skorost > 0 поднимается после zaderzhka (промывка-погоня)
  | 'iney' // зона: принудительная Корка без урона; zaderzhka секунд внутри до Корки (0 камера, 3 иней на стене)
  ...
  | 'kriostat' // опыт яруса 1: бак на гусеницах, x левый низ, w,h; рельс от x до x+hodX; один на уровень
  ...
  | 'uborshchik' // дрон: патруль по полу, толкает героя щёткой, не жжёт; замыкает в воде
